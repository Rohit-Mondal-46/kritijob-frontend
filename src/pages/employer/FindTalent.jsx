...
//         location: ''
//     });
//     const [page, setPage] = useState(1); // Initialize to 1, not 0
//     const [total, setTotal] = useState(0);
//     const { addToast } = useToast();
//     const navigate = useNavigate();
//     const { user } = useContext(AuthContext);
...
//         // Debounce search
//         const timer = setTimeout(() => {
//             // Reset to page 1 when filters change (already handled in handleFilterChange)
//             fetchCandidates();
//         }, 500);
//         return () => clearTimeout(timer);
//     }, [filters, page]);
...
//         // Check if there are more pages (if we got full page of results)
...
// FindTalent.tsx (updated pagination section)
...
const FindTalent = () => {
    ...
    const fetchCandidates = useCallback(async () => {
        ...
        try {
            setLoading(true);
            const queryParams = new URLSearchParams({
                page: currentPage.toString(), // Ensure page is sent as string
                limit: '12',
    ...
    return (
        <div className={styles.container}>
            ...
            {/* Candidate Profile Modal - keep as is */}
            {selectedCandidate && (
                <div className={styles.modalOverlay} onClick={closeModal}>
                    {/* ... modal content ... */}
                </div>
